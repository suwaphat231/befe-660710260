...
func createStudent(c *gin.Context) {
	var newStundent Student

	if err := c.ShouldBindJSON(&newStundent); err != nil {
	...
	if newStundent.Year < 1 || newStundent.Year > 4 {
		c.JSON(http.StatusBadRequest, gin.H{"error": "year must be between 1-4"})
		return

	}
	newStundent.ID = fmt.Sprintf("%d", len(students)+1)

	students = append(students, newStundent)
	c.JSON(http.StatusOK, students)
}
